...
import { Package, AlertCircle, ShieldCheck, Plus, Search, Filter, Wallet, FileDown, TrendingUp, X, Trophy, Share2, MessageCircle, Clock, BellRing, PieChart as ChartIcon, CheckCircle2, HeartHandshake, FolderOpen, BarChart3, Plane, QrCode, Lock, ArrowDownRight, ArrowUpRight, Calculator } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const exportIRPF = () => {
    if (!profile?.is_premium) {
      toast.error('O Relatório de IRPF é um recurso Pro. Faça o upgrade!');
      return;
    }
    const doc = new jsPDF();
    doc.setFontSize(20); doc.setTextColor(15, 23, 42); doc.text('Relatório Auxiliar para IRPF 2026', 14, 20);
    doc.setFontSize(10); doc.setTextColor(100); doc.text('Este documento auxilia no preenchimento da ficha de Bens e Direitos.', 14, 28);
    
    const tableData = filteredWarranties.map(w => [
      w.name,
      `Adquirido em ${formatDate(w.purchase_date)} na loja ${w.store || 'Não Informada'}.`,
      `R$ ${Number(w.price || 0).toLocaleString('pt-BR')}`
    ]);

    autoTable(doc, {
      startY: 35,
      head: [['Bem / Produto', 'Discriminação Sugerida', 'Situação em 31/12/2025']],
      body: tableData,
      headStyles: { fillColor: [15, 23, 42] }
    });

    doc.save(`irpf-guardiao-${new Date().getFullYear()}.pdf`);
    toast.success('Relatório de IRPF gerado!');
  };
    ...
    <div className="space-y-10 pb-32">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex flex-wrap gap-3">
          <Button variant="outline" onClick={exportIRPF} className="gap-2 border-slate-200 text-slate-600 font-bold shadow-sm">
            ...
            <AnimatePresence mode="wait">
              <motion.div key={currentAssetIndex} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex flex-col md:flex-row items-center justify-between gap-8 relative z-10">
                <div className="space-y-4 text-center md:text-left">
                  <div className="flex items-center justify-center md:justify-start gap-2 text-emerald-400 font-black text-[10px] uppercase tracking-widest"><TrendingUp className="h-4 w-4" /> Insight de Patrimônio</div>
                  <h3 className="text-3xl font-black leading-tight max-w-lg">Seu <span className="text-emerald-400">{warranties[currentAssetIndex].name}</span> vale R$ {getDepreciatedValue(warranties[currentAssetIndex]).toLocaleString('pt-BR')} hoje.</h3>
                  <p className="text-slate-400 text-sm font-medium">Hora de trocar? Recupere valor vendendo este mês.</p>
                </div>
                <div className="flex gap-4">
                  <Link href={`/products/${warranties[currentAssetIndex].id}`}><Button variant="ghost" className="text-white hover:bg-white/10 font-bold border border-white/10 px-8 h-14 rounded-2xl uppercase text-[10px] tracking-widest">Ver Dossiê</Button></Link>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        <Card className="border-teal-100 bg-white shadow-xl p-8 flex flex-col justify-center text-center md:text-left">
          <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-2 flex items-center justify-center md:justify-start gap-2"><ShieldCheck className="h-4 w-4 text-emerald-600" /> Saúde Patrimonial</p>
          <div className="text-4xl font-black text-slate-900">{safetyScore}%</div>
          <div className="mt-2 h-1.5 w-full bg-slate-100 rounded-full overflow-hidden"><motion.div initial={{ width: 0 }} animate={{ width: `${safetyScore}%` }} className="h-full bg-emerald-500" /></div>
        </Card>
        <Card className="border-teal-100 bg-white shadow-xl p-8 flex flex-col justify-center text-center md:text-left"><p className="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-2">Total Investido (Original)</p><div className="text-4xl font-black text-slate-900">R$ {totalOriginalValue.toLocaleString('pt-BR')}</div><p className="text-[10px] text-slate-400 font-bold uppercase mt-2">Valor acumulado em notas</p></Card>
        <Card className="border-teal-100 bg-emerald-600 text-white shadow-xl p-8 flex flex-col justify-center text-center md:text-left relative overflow-hidden"><div className="relative z-10"><p className="text-[10px] font-black uppercase text-emerald-100 tracking-widest mb-2 flex items-center justify-center md:justify-start gap-2"><ArrowUpRight className="h-4 w-4" /> Liquidez Total</p><div className="text-4xl font-black">R$ {totalCurrentValue.toLocaleString('pt-BR')}</div><p className="text-[10px] text-emerald-100 font-bold uppercase mt-2">Valor de revenda somado hoje</p></div></Card>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2 text-teal-600 font-bold text-[10px] uppercase tracking-tighter mr-2"><FolderOpen className="h-3.5 w-3.5" /> Pastas:</div>
