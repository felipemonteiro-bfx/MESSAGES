...
import { Package, AlertCircle, ShieldCheck, Plus, Search, Filter, Wallet, FileDown, TrendingUp, X, Trophy, Share2, MessageCircle, Clock, BellRing, PieChart as ChartIcon, CheckCircle2, HeartHandshake, FolderOpen, BarChart3, Plane, QrCode, Lock, ArrowDownRight } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  // Função interna de depreciação para o dashboard
  ...
  const toggleSelect = (id: string) => {
    if (!profile?.is_premium && selectedItems.length >= 2) {
      toast.error('Limite gratuito atingido (2 itens).');
      return;
    }
  ...
  const exportPDF = (items = filteredWarranties) => {
    if (!profile?.is_premium && items.length > 5) {
      toast.error('Limite Pro: Selecione no máximo 5 itens.');
      return;
    }
    const doc = new jsPDF();
    ...
    <div className="space-y-10 pb-32">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex flex-wrap gap-3">
          <Button variant="outline" onClick={() => exportPDF()} className="gap-2 border-emerald-100 text-emerald-700 font-bold"><FileDown className="h-4 w-4" /> Dossiê PDF</Button>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        <Card className="md:col-span-2 bg-slate-900 text-white border-none overflow-hidden relative shadow-2xl">
          ...
          <CardContent className="p-8 flex flex-col md:flex-row items-center gap-8 relative z-10">
            ...
            <div className="space-y-4 text-center md:text-left flex-1">
              <div><h3 className="text-xl font-bold flex items-center justify-center md:justify-start gap-2"><Trophy className="h-5 w-5 text-amber-400" /> Nível de Proteção</h3><p className="text-sm text-slate-400 font-medium">Sua saúde patrimonial está {safetyScore > 80 ? 'excelente' : 'em atenção'}.</p></div>
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-white/5 p-3 rounded-2xl border border-white/10 text-left">
                  <p className="text-[9px] font-black uppercase text-slate-500">Total Original</p>
                  <p className="text-lg font-black text-white">R$ {totalOriginalValue.toLocaleString('pt-BR')}</p>
                </div>
                <div className="bg-white/5 p-3 rounded-2xl border border-white/10 text-left">
                  <p className="text-[9px] font-black uppercase text-emerald-500 flex items-center gap-1">Valor Atual <ArrowDownRight className="h-3 w-3" /></p>
                  <p className="text-lg font-black text-emerald-400">R$ {totalCurrentValue.toLocaleString('pt-BR')}</p>
                </div>
              </div>
            </div>
          </CardContent>
        ...
        <Card className="border-teal-100 bg-white shadow-xl overflow-hidden">
          <CardHeader className="pb-0 bg-slate-50/50 border-b border-slate-100 py-4"><CardTitle className="text-[10px] font-black uppercase text-slate-400 flex items-center gap-2"><BarChart3 className="h-3 w-3 text-emerald-600" /> Alocação por Pasta</CardTitle></CardHeader>
          <CardContent className="h-48 pt-6">
            <ResponsiveContainer width="100%" height="100%"><BarChart data={folderData}><XAxis dataKey="name" hide /><YAxis hide /><Tooltip contentStyle={{ borderRadius: '12px', border: 'none' }} formatter={(v: any) => [`R$ ${v.toLocaleString('pt-BR')}`]} /><Bar dataKey="valor" fill="#059669" radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer>
          </CardContent>
        </Card>
      </div>
